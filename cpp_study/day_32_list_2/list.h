...
namespace study_list {
    ...
    public:
        ...
        list() {
            _head = new Node();
            _head->next = _head;  // 初始化头节点的next指向自己
            _head->prev = _head;  // 初始化头节点的prev指向自己
        }
        ...
        void erase(iterator it) {
            Node* node_to_delete = it._node; // 取得要删除的节点
            node_to_delete->prev->next = node_to_delete->next; // 连接前后节点
            node_to_delete->next->prev = node_to_delete->prev;
            delete node_to_delete; // 删除节点
        }

        // 在指定位置插入节点
        void insert(iterator pos, const T& data) {
            Node* node_to_insert = new Node(data); // 创建新节点
            Node* cur = pos._node;                  // 获取当前节点
            Node* prev = cur->prev;                 // 获取前驱节点

            prev->next = node_to_insert; // 连接前驱节点与新节点
            node_to_insert->prev = prev;  // 新节点的prev指向前驱节点
            node_to_insert->next = cur;   // 新节点的next指向当前节点
            cur->prev = node_to_insert;    // 当前节点的prev指向新节点
        }

...
def get_url():
    ...
    for n in range(int(page)):
        url = f'https://proxy.ip3366.net/free/?action=china&page={n+1}'
        url_list.append(url)
    return url_list
    ...
    try:
        ...
        async with aiohttp.ClientSession(headers=heads,timeout=timeout) as session:
            async with session.get(url) as resp:
                ...
                ip = tree.xpath('//*[@id="content"]/section/div[2]/table/tbody/tr/td[1]/text()|//*[@id="content"]/section/div[2]/table/tbody/tr/td[2]/text()')
                ip_list = []
                path = '代理池'
                for i in ip:
